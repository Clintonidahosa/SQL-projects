-- # SQL Code for Hospital Readmission Data Cleaning and Analysis

## Step 1: Duplicate the table for backup purposes (Data Inspection)
CREATE TABLE hosp_read.readmission_backup AS
SELECT * FROM hosp_read.readmission;

-- Verify if the table has been duplicated
SELECT COUNT(*) AS row_count FROM hosp_read.readmission_backup;

## Step 2: Remove rows where critical columns have 'N/A' or null values (Data Cleaning)
...
-- Convert 'N/A' in Number of Discharges to NULL for accurate processing
...
## Step 3: Modify the schema to change data types (Data Inspection)
-- Ensures numeric columns are correctly defined for calculations
...
## Step 4: Set negative values to NULL for readmission-related columns (Data Cleaning)
-- Negative values are invalid and should be treated as missing data
UPDATE hosp_read.readmission
...
-- Aggregates data to explore key metrics
...
## Step 6: Filter hospitals with readmission ratio above 1.0 and more than 500 discharges (Data Exploration)
-- Identifies facilities with high readmission rates and substantial discharges
...
## Step 7: Filter records based on the reporting date range (Data Exploration)
-- Selects data within a specific time range for further analysis
SELECT `Facility Name`, `State`, `Start Date`, `End Date`
FROM hosp_read.readmission
WHERE `Start Date` >= '2019-01-01' AND `End Date` <= '2022-12-31';

## Step 8: Subquery - Find hospitals with readmission ratios higher than the state average (Data Exploration)
-- Compares individual hospital readmission ratios to their state averages
...
## Step 9: Compute a running total of discharges per state (Data Exploration)
-- Uses window functions to track cumulative discharges per state
...
## Step 10: Compute moving average of Excess Readmission Ratio for each state (Data Exploration)
-- Calculates a moving average of readmission ratios to smooth data
SELECT `Facility Name`, `State`, `Excess Readmission Ratio`,
...
## Step 11: Create indexes on common search columns (Data Optimization)
...
## Step 12: Add constraints to ensure valid data in critical columns (Data Validation)
-- Ensures key metrics cannot have invalid values
ALTER TABLE hosp_read.readmission
...
## Step 13: Calculate average readmission ratio per year based on the 'Start Date' (Data Exploration)
-- Tracks trends in readmission ratios over time
SELECT YEAR(`Start Date`) AS year, AVG(`Excess Readmission Ratio`) AS avg_readmission_ratio
FROM hosp_read.readmission
GROUP BY year
ORDER BY year;

## Step 14: Summary report of hospitals with the highest average readmission ratios per state (Data Exploration)
-- Highlights top 10 hospitals with the worst readmission rates
...
## Step 15: Report on hospitals with readmission ratios higher than the national average (Data Exploration)
-- Identifies hospitals that perform worse than the national average
SELECT `Facility Name`, `State`, `Excess Readmission Ratio`
FROM hosp_read.readmission
